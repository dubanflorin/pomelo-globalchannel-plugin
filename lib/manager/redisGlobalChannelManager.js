...
GlobalChannelManager.prototype.leaveAllChannels = function (uid, sid, cb) {
  ...
  self.redis.smembers(genPlayerChannelsKey(self, uid, sid), function (err, list) {
    ...
    for(var i=0; i<list.length; i++) {
      commands.push(['srem', genKey(self, list[i], sid), uid]);
    }
...
GlobalChannelManager.prototype.getMembersBySid = function(names, sid, cb) {
  if (!Array.isArray(names)) names = [names]
  const args = names.map(name => genKey(this, name, sid))
  this.redis.sunion(args, function(err, list) {
